...
export class MedicineRepository {
  ...
  async create(params: CreateMedicineParams) {
    const [newMedicine] = await this.db
  ...
  async findMany(query: FindManyMedicineParams) {
    const order =
      query.sort_by_name === 'desc'
        ? desc(schema.medicines.name)
        : asc(schema.medicines.name);
    const filters: SQL[] = [];

    if (query.search_by_name)
      filters.push(ilike(schema.medicines.name, `%${query.search_by_name}%`));
    if (query.search_by_id)
      filters.push(eq(schema.medicines.id, query.search_by_id));
  ...
  async update(params: UpdateMedicineParams) {
    const [updatedMedicine] = await this.db
  ...
  async decrementStock(id: string, amount: number, tx?: any) {
    const executor = tx || this.db;
    const med = await executor.query.medicines.findFirst({
      where: eq(schema.medicines.id, id),
    });
    const newStock = Math.max((med?.stock ?? 0) - amount, 0);
