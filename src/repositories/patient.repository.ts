...
export class PatientRepository {
    ...
    async findMany(query: FindManyPatientParams) {
        const order = query.sort_by_name === 'desc' ? desc(schema.patients.name) : asc(schema.patients.name);
        const filters: SQL[] = [];

        if (query.search_by_name) filters.push(ilike(schema.patients.name, `%${query.search_by_name}%`));
        if (query.search_by_id) filters.push(eq(schema.patients.id, query.search_by_id));
    ...
    async update(params: UpdatePatientParams) {
        const [updatedPatient] = await this.db.update(schema.patients)
            .set({
                nationalId: params.national_id,
                name: params.name,
                dob: params.birth_date ? new Date(params.birth_date).toISOString().split('T')[0] : undefined,
